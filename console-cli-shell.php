<?php
  ...
  */
  ...
  class TBWorker extends Collectable {
    ...
    public function __construct($id, $cmd = 'startBot.bat') {
        $this->id = $id;
        $this->cmd = $cmd;
        $this->run = true;
        $this->uuid = uniqid();
        if(file_exists(LOG_PATH.DIRECTORY_SEPARATOR.'worker-'.$this->id.'.log')) {
          unlink(LOG_PATH.DIRECTORY_SEPARATOR.'worker-'.$this->id.'.log');
        }
        $this->log('Worker: '.$this->uuid.' has been constructed');
    }
    
    public function run() {
      $this->log('Worker: '.$this->uuid.' has started');
      $OperatingSystem = php_uname('s'); 
      if ($OperatingSystem == "Windows NT") {  
        $descriptorspec = array ( 0 => array("pipe", "r"), 1 => array("pipe", "w") );  
        
        if(is_resource($prog = proc_open('"'.$this->cmd.' '.$this->id.' > '.LOG_PATH.DIRECTORY_SEPARATOR.'shell-'.$this->id.'.log 2>&1"', $descriptorspec, $pipes, getcwd(), NULL))) {  
          $ppid = proc_get_status($prog);  
          $this->ppid=$ppid['pid'];
          
          $output = array_filter(explode(" ", shell_exec("wmic process get parentprocessid,processid | find \"".$this->ppid."\"")));  
          array_pop($output);  
        
          $this->pid = end($output);
          $this->log('Worker: '.$this->uuid.' has PID: '.$this->ppid. ' and PHP has PID: '.$this->pid);
          while ($this->check()) { sleep(5); };
        }  
      }  
      $this->log('Worker: '.$this->uuid.' has finished');
    }
    
    function check() {
      if($this->run) {
        $output = array_filter(explode(" ", shell_exec('wmic process where "ProcessID = '.$this->ppid.'" get processid')));  
        if(!isset($output[2])) {
          $this->log('Worker: '.$this->uuid.' has crashed');
          $this->run = false;
        }
      }
      return $this->run;
    ...
    function log($msg) {
      $msg = str_replace(array(chr(10), chr(13)), '', $msg);
      file_put_contents(LOG_PATH.DIRECTORY_SEPARATOR.'worker-'.$this->id.'.log', $msg.PHP_EOL, FILE_APPEND | LOCK_EX);
    }
    
    public function destroy() {
      if(isset($this->ppid)) {
        exec("taskkill /pid ".$this->ppid." /F");
      }
      if(isset($this->pid)) {
        exec("taskkill /pid ".$this->pid." /F");
      }
      $this->run = false;
      $this->log('Worker: '.$this->uuid.' has been interupted');
    }
  }
